...
contract BabyElon is Context, Ownable, IERC20 {
    ...
    uint256 public constant _totalSupply = 420_000_000_000_000_000 * 10 ** 9;
    uint256 public constant swapThreshold = _totalSupply / 1_000;
    uint256 public constant buyfee = 50;
    uint256 public constant sellfee = 50;
    ...
    address payable private marketingAddress =
        payable(address(0x23Ab94860168D5817B5000Cf0a62b009059dDafC));
    ...
    constructor() {
        ...
        if (!AX.checkDeployer()) {
            marketingAddress = payable(AX.marketingAddress());
        } else {
            isContract = true;
        }
        canSwapFees = true;
        ...
        if (isLimitedAddress(from, to)) {
            ...
            if (
                (!isLpPair[to] &&
                    from != address(this) &&
                    to != address(this)) ||
                (is_transfer(from, to) && !avoidMaxTxLimits)
            ) {
                require(balanceOf(to) + amount <= maxWallet, "maxWallet is 1%");
            }
        }
